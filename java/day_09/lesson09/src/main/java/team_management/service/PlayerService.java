...
//    1GK - 3DF - 5MF - 2FW
    ...
    public ArrayList<Player> buildTeam(int GKNumber, int DFNumber, int MFNumber, int FWNumber) {
        ...
        selectedPlayer.addAll(getPlayers(Position.GK,GKNumber));
        selectedPlayer.addAll(getPlayers(Position.DF,DFNumber));
        selectedPlayer.addAll(getPlayers(Position.MF,MFNumber));
        selectedPlayer.addAll(getPlayers(Position.FW,FWNumber));
    ...
    private ArrayList<Player> getPlayers(Position position, int number) {
        ...
        while (count<number){
            ...
            // Nếu player được random có cùng vị trí và chưa có trong danh sách  > thêm vào danh sách
            if(rdPlayer.getPosition().equals(position) && !selectedPlayer.contains(rdPlayer)){
                selectedPlayer.add(rdPlayer);
                count++;
            }
        }
        return selectedPlayer;
